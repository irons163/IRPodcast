...
@implementation IRHCPathTool

+ (NSString *)rootDirectory
{
    static NSString *obj = @"IRHTTPCache";
...
+ (NSString *)logPath
{
    NSString *path = [[self rootDirectory] stringByAppendingPathComponent:@"IRHTTPCache.log"];
    return [self converToAbsoultePath:path];
}

+ (NSString *)archivePath
{
    NSString *path = [[self rootDirectory] stringByAppendingPathComponent:@"IRHTTPCache.archive"];
    return [self converToAbsoultePath:path];
}

+ (NSString *)directoryPathWithURL:(NSURL *)URL
{
    NSString *name = [[IRHCURLTool tool] keyWithURL:URL];
...
{
    NSString *fileName = [[[IRHCURLTool tool] keyWithURL:URL] stringByAppendingPathExtension:URL.pathExtension];
...
{
    NSString *baseFileName = [[IRHCURLTool tool] keyWithURL:URL];
    NSString *directoryPath = [self directoryPathWithURL:URL];
    int number = 0;
